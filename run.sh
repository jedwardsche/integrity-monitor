...
cd "$PROJECT_ROOT"
PYTHONPATH="$PROJECT_ROOT" uvicorn backend.main:app --reload --reload-exclude "**/.venv/**" --reload-exclude "**/__pycache__/**" --reload-exclude "**/.git/**" --reload-exclude "**/node_modules/**" --host 0.0.0.0 --port $BACKEND_PORT > "$PROJECT_ROOT/.backend.log" 2>&1 &
BACKEND_PID=$!
